...
The Sheets API can return `90235: Data not ready`. The CLI currently does not automatically retry this business error; wait and rerun the read. For embedded tables, check warnings and retained placeholders before treating the document as complete.
...
A successful export confirms the download, not formula compatibility with Excel. In live verification, Feishu `IMPORTRANGE` formulas remained in the exported workbook and produced `#NAME?` in Excel, propagating to dependent totals and ratios. The CLI does not translate formulas or embed their external source data. Recalculation alone cannot fix unsupported functions. Exported formula cells may also lack cached results, so readers such as `openpyxl` with `data_only=True` can return empty values even though formulas exist. Supported formulas need a compatible calculation engine to populate those results.

Export requires either `docs:document:export` or `drive:export:readonly`; one is sufficient ([official API permissions](https://open.feishu.cn/document/server-docs/docs/drive-v1/export_task/create)). Enabling a scope in the app console does not grant it to an existing user access token. If `docs:document:export` is already enabled, run `feishu-docs authorize --scope "docs:document:export"` to authorize the user token, then retry the export; there is no need to add both scopes.
