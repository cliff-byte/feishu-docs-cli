...
export async function search(args, globalOpts) {
  ...
  const docType = args.type || undefined;
  ...
  while (results.length < limit) {
    const body = { search_key: query, count: pageSize, offset };
    ...
    if (!res?.data?.has_more || items.length === 0) break;
    offset += items.length;
  }
