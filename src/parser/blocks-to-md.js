 ...
 * @param {object[]} blocks - Flat block array from Feishu API
 * @param {object} [options] - { imageUrlMap: Map<fileToken, url> }
 */
...
function renderNode(node, lines, ctx, depth, state) {
  ...
  if (type === BlockType.CODE) {
    ...
    const lang = CODE_LANGUAGES[langNum] || "";
  ...
  if (type === BlockType.IMAGE) {
    ...
    const url = ctx.imageUrlMap.get(fileToken) || "";
    ...
    const data =
      node[
        Object.keys(node).find(
          (k) =>
            typeof node[k] === "object" && node[k] !== null && node[k]?.token,
        )
      ] || {};
...
function cellToText(cell, ctx) {
  ...
  for (const child of cell._children) {
    const key = Object.keys(child).find(
      (k) => typeof child[k] === "object" && child[k]?.elements,
    );
    if (key) {
      parts.push(elementsToMarkdown(child[key].elements, ctx));
    }
  }
